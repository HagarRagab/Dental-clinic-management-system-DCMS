...
import {
    ...
    calendarDentists,
    mockAppointments,
} from "@/features/appointments/services/mock-calendar-service";
...
export function CalendarScreen({ initialRole }: { initialRole: UserRole }) {
    ...
    const [selected, setSelected] = useState<CalendarAppointment>(
        mockAppointments[0],
    );
    const dentists =
        role === "dentist"
            ? ["Dr. Karim Mostafa"]
            : dentistFilter === "All dentists"
              ? calendarDentists.slice(1)
              : [dentistFilter];
        ...
        >
            <div className="calendar-page-heading">
                <div>
                    ...
                    <h1>Calendar</h1>
                    <p>
                        Keep the day moving without losing the clinical context.
                    </p>
                </div>
                {role !== "dentist" ? (
                    <Link
                        href={`/appointments/new?role=${role}`}
            ...
            >
                <div className="calendar-toolbar__date">
                    ...
                    <button type="button" className="calendar-date-button">
                        Today <span>Saturday, 13 September</span>
                    </button>
                    <button
                        type="button"
                        className="icon-button"
            ...
            <div className="calendar-layout">
                ...
                >
                    <div className="calendar-legend">
                        ...
                        <span>
                            <i className="legend-dot legend-dot--waiting" />
                            Visit state: Waiting
                        </span>
                    </div>
                    <div className={`calendar-board calendar-board--${view}`}>
                        <div className="calendar-board__header">
                            <span>Time</span>
                            {dentists.map((dentist) => (
                                <strong key={dentist}>
                                    {view === "week"
                                        ? `${dentist.split(" ").slice(0, 2).join(" ")} · Sat 13`
                                        : dentist}
                                </strong>
                            ))}
                        </div>
                ...
                <AppointmentDetailPanel appointment={selected} role={role} />
            </div>
        </AppShell>
    );
}
